...
namespace Rewired.Demos
{
	...
	public class ControlRemappingDemo1 : MonoBehaviour
	{
		...
		{
			if (dialog.enabled)
				return; // don't allow closing the menu while dialog is open so there won't be issues remapping the Menu button
		...
		private void DrawPlayerSelector ()
		{
			...
			foreach (Player player in ReInput.players.GetPlayers(true)) {
				if (selectedPlayer == null)
					selectedPlayer = player;  // if no player selected, select first
		...
		private void DrawMouseAssignment ()
		{
			...
			if (GUI.enabled != origGuiEnabled)
				GUI.enabled = origGuiEnabled; // restore GUI state
		}
		...
		private void DrawControllerSelector ()
		{
			...
			if (!selectedPlayer.controllers.hasMouse)
				GUI.enabled = false; // disable mouse if player doesn't have mouse assigned
			...
			if (GUI.enabled != origGuiEnabled)
				GUI.enabled = origGuiEnabled; // re-enable gui
		...
		private void DrawCategoryActions ()
		{
			...
			bool origGuiEnabled = GUI.enabled; // save GUI state
			if (selectedMap == null)
				return; // controller map does not exist for this category in this controller
			...
			if (actionCategory == null)
				return; // no action category exists with the same name
			...
			foreach (InputAction action in ReInput.mapping.ActionsInCategory(actionCategory.id)) {
				...
				if (action.type == InputActionType.Button) {
					...
					if (selectedController.type != ControllerType.Keyboard) { // don't draw this for keyboards since keys can only be assigned to the +/- axes anyway
						...
						foreach (ActionElementMap elementMap in selectedMap.AllMaps) {
							...
							if (elementMap.elementType == ControllerElementType.Button)
								continue; // skip buttons, will handle below
							if (elementMap.axisType == AxisType.Split)
								continue; // skip split axes, will handle below
							DrawActionAssignmentButton (selectedPlayer.id, action, Pole.Positive, selectedController, selectedMap, true, elementMap);
					...
					foreach (ActionElementMap elementMap in selectedMap.AllMaps) {
						if (elementMap.actionId != action.id)
							continue;
						if (elementMap.axisContribution != Pole.Positive)
							continue; // axis contribution is incorrect, skip
						if (elementMap.axisType == AxisType.Normal)
							continue; // normal axes handled above
					...
					foreach (ActionElementMap elementMap in selectedMap.AllMaps) {
						if (elementMap.actionId != action.id)
							continue;
						if (elementMap.axisContribution != Pole.Negative)
							continue; // axis contribution is incorrect, skip
						if (elementMap.axisType == AxisType.Normal)
							continue; // normal axes handled above
		...
		{
			if (!dialog.enabled)
				return; // prevent this from running after dialog is closed
		...
		private void DrawCalibrationWindow (string title, string message)
		{
			...
			if (entry.recording)
				GUI.enabled = false; // don't allow switching while recording min/max
			...
			if (GUI.enabled != origGUIEnabled)
				GUI.enabled = origGUIEnabled; // restore gui
		...
		private void DialogResultCallback (int queueActionId, UserResponse response)
		{
			foreach (QueueEntry entry in actionQueue) { // find the right one and cancel or confirm it
				...
				if (response != UserResponse.Cancel)
					entry.Confirm (response); // mark the entry as confirmed and record user response
                else
					entry.Cancel (); // mark the entry as canceled
				break;
			}
		}
		...
		{
			if (dialog.busy)
				return; // do not allow assignment until dialog is ready
		...
		private void PollKeyboardForAssignment (ElementAssignmentChange entry)
		{
			...
			foreach (ControllerPollingInfo info in ReInput.controllers.Keyboard.PollForAllKeys()) {
				...
				if (key == KeyCode.AltGr)
					continue; // skip AltGr key because it gets fired when alt and control are held on some keyboards
				...
				if (Keyboard.IsModifierKey (info.keyboardKey)) { // a modifier key is pressed
					if (modifierPressedCount == 0)
						firstModifierKeyInfo = info; // store the polling info for the first modifier key pressed in case its the only key pressed
					...
					if (nonModifierKeyInfo.keyboardKey != KeyCode.None)
						continue; // skip after the first one detected, we only need one non-modifier key press
		...
		{
			if (dialog.enabled)
				return; // dialog is open, do not process queue
		...
		private bool ProcessJoystickAssignmentChange (JoystickAssignmentChange entry)
		{
			...
			if (player.controllers.ContainsController (ControllerType.Joystick, entry.joystickId))
				return true; // same player, nothing to change
		...
		private bool ProcessAddOrReplaceElementAssignment (ElementAssignmentChange entry)
		{
			...
			if (entry.state == QueueEntry.State.Confirmed) { // the action assignment has been confirmed
				if (Event.current.type != EventType.Layout)
					return false; // only make changes in layout to avoid GUI errors when new controls appear
			...
			for (int i = 0; i < allPlayers.Count; i++) {
				...
				for (int j = 0; j < behaviors.Count; j++) {
					...
					if (xml == null || xml == string.Empty)
						continue; // no data found for this behavior
				...
				if (keyboardMaps.Count > 0)
					player.controllers.maps.ClearMaps (ControllerType.Keyboard, true); // clear only user-assignable maps, but only if we found something to load. Don't _really_ have to clear the maps as adding ones in the same cat/layout will just replace, but let's clear anyway.
				...
				if (foundJoystickMaps)
					player.controllers.maps.ClearMaps (ControllerType.Joystick, true); // clear only user-assignable maps, but only if we found something to load. Don't _really_ have to clear the maps as adding ones in the same cat/layout will just replace, but let's clear anyway.
				int count = 0;
				foreach (Joystick joystick in player.controllers.Joysticks) {
				...
				// Mouse Maps
				if (mouseMaps.Count > 0)
					player.controllers.maps.ClearMaps (ControllerType.Mouse, true); // clear only user-assignable maps, but only if we found something to load. Don't _really_ have to clear the maps as adding ones in the same cat/layout will just replace, but let's clear anyway.
		...
		private void LoadJoystickMaps (int joystickId)
		{
			...
			for (int i = 0; i < allPlayers.Count; i++) { // this controller may be owned by more than one player, so check all
				...
				if (!player.controllers.ContainsController (ControllerType.Joystick, joystickId))
					continue; // player does not have the joystick
		...
		private string GetControllerMapXml (Player player, ControllerType controllerType, int categoryId, int layoutId, Controller controller)
		{
			...
			if (!PlayerPrefs.HasKey (key))
				return string.Empty; // key does not exist
		...
		private class DialogHelper
		{
			...
			// a small delay after opening the window that prevents assignment input for a short time after window opens
			private const float closeBusyDelay = 0.1f;
			// a small after closing the window that the GUI will still be busy to prevent button clickthrough
				...
				set {
					if (value) {
						if (_type == DialogType.None)
							return; // cannot enable, no type set
			...
			public void StartModal (int queueActionId, DialogType type, WindowProperties windowProperties, Action<int, UserResponse> resultCallback, float closeTimer, float openBusyDelay)
			{
				...
				if (openBusyDelay >= 0.0f)
					StateChanged (openBusyDelay); // override with user defined open busy delay
			}
			...
			public void DrawConfirmButton (string title)
			{
				...
				if (busy)
					GUI.enabled = false; // disable GUI if dialog is busy to prevent click though
